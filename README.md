...
| Case 4: HTTP/10 threads/No connection pooling  | ![](img/HTTP10ThreadsNoPool.png) | 827                          | 370.680                                  | 236.513                       <br/> | In theory this should be the slowest however the performance is on par with cases 2 and 3 with overall query time and somehow better in servlet and jdbc times.           |
...
| Case 3: HTTP/10 threads/No connection pooling  | ![](img/scaled_10_thread_no_conn_pool.png)| 281                        | 187.854                             | 118.909                    <br/> | This is the slowest time of them all since there is no connection pooling and the time to spin up a new connection takes up time. Also this is the one with the highest standard deviation for the query time.            |
